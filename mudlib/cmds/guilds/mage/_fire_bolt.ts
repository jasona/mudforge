/**
 * Fire Bolt - Mage combat skill.
 * A bolt of fire that burns the target.
 */
...
export const name = ['fire_bolt', 'firebolt', 'fb'];
export const description = 'A bolt of fire that burns the target';
export const usage = 'fire_bolt [target]';

export async function execute(ctx: CommandContext): Promise<void> {
  const guildDaemon = getGuildDaemon();
  const player = ctx.player as GuildPlayer;
  const skillId = 'mage:fire_bolt';

  if (!guildDaemon.hasSkill(player, skillId)) {
    ctx.sendLine('{yellow}You have not learned Fire Bolt yet.{/}');
    return;
  }
