/**
 * Magic Missile - Mage combat skill.
 * A bolt of pure magical energy that never misses.
 */
...
export const name = ['magic_missile', 'magicmissile', 'mm'];
export const description = 'A bolt of pure magical energy that never misses';
export const usage = 'magic_missile [target]';

export async function execute(ctx: CommandContext): Promise<void> {
  const guildDaemon = getGuildDaemon();
  const player = ctx.player as GuildPlayer;
  const skillId = 'mage:magic_missile';

  if (!guildDaemon.hasSkill(player, skillId)) {
    ctx.sendLine('{yellow}You have not learned Magic Missile yet.{/}');
    return;
  }
