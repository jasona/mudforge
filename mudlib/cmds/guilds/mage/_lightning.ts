/**
 * Lightning Bolt - Mage combat skill.
 * A crackling bolt of lightning that strikes instantly.
 */
...
export const name = ['lightning', 'lightning_bolt', 'lightningbolt', 'lb'];
export const description = 'A crackling bolt of lightning that strikes instantly';
export const usage = 'lightning [target]';

export async function execute(ctx: CommandContext): Promise<void> {
  const guildDaemon = getGuildDaemon();
  const player = ctx.player as GuildPlayer;
  const skillId = 'mage:lightning';

  if (!guildDaemon.hasSkill(player, skillId)) {
    ctx.sendLine('{yellow}You have not learned Lightning Bolt yet.{/}');
    return;
  }
