/**
 * Teleport - Mage utility skill.
 * Instantly teleport to a known location.
 */
...
export const name = ['teleport', 'tele'];
export const description = 'Instantly teleport to a known location';
export const usage = 'teleport [location]';

export async function execute(ctx: CommandContext): Promise<void> {
  const guildDaemon = getGuildDaemon();
  const player = ctx.player as GuildPlayer;
  const skillId = 'mage:teleport';

  if (!guildDaemon.hasSkill(player, skillId)) {
    ctx.sendLine('{yellow}You have not learned Teleport yet.{/}');
    return;
  }
