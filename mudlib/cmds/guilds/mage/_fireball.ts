/**
 * Fireball - Mage combat skill.
 * A massive explosion of fire that damages all enemies.
 */
...
export const description = 'A massive explosion of fire that damages all enemies';
export const usage = 'fireball';

export async function execute(ctx: CommandContext): Promise<void> {
  const guildDaemon = getGuildDaemon();
  const player = ctx.player as GuildPlayer;
  const skillId = 'mage:fireball';

  if (!guildDaemon.hasSkill(player, skillId)) {
    ctx.sendLine('{yellow}You have not learned Fireball yet.{/}');
    return;
  }
