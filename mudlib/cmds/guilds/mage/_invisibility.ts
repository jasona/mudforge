/**
 * Invisibility - Mage buff skill.
 * Become invisible to normal sight.
 */
...
export const name = ['invisibility', 'invis'];
export const description = 'Become invisible to normal sight';
export const usage = 'invisibility';

export async function execute(ctx: CommandContext): Promise<void> {
  const guildDaemon = getGuildDaemon();
  const player = ctx.player as GuildPlayer;
  const skillId = 'mage:invisibility';

  if (!guildDaemon.hasSkill(player, skillId)) {
    ctx.sendLine('{yellow}You have not learned Invisibility yet.{/}');
    return;
  }
