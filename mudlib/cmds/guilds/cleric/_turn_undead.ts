/**
 * Turn Undead - Cleric combat skill.
 * Deal holy damage to undead creatures.
 */
...
export const name = ['turn_undead', 'turnundead', 'turn'];
export const description = 'Deal holy damage to undead creatures';
export const usage = 'turn_undead [target]';

export async function execute(ctx: CommandContext): Promise<void> {
  const guildDaemon = getGuildDaemon();
  const player = ctx.player as GuildPlayer;
  const skillId = 'cleric:turn_undead';

  if (!guildDaemon.hasSkill(player, skillId)) {
    ctx.sendLine('{yellow}You have not learned Turn Undead yet.{/}');
    return;
  }
