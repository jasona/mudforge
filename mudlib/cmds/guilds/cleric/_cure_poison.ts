/**
 * Cure Poison - Cleric utility skill.
 * Remove poison effects from a target.
 */
...
export const name = ['cure_poison', 'curepoison', 'cure'];
export const description = 'Remove poison effects from a target';
export const usage = 'cure_poison [target]';

export async function execute(ctx: CommandContext): Promise<void> {
  const guildDaemon = getGuildDaemon();
  const player = ctx.player as GuildPlayer;
  const skillId = 'cleric:cure_poison';

  if (!guildDaemon.hasSkill(player, skillId)) {
    ctx.sendLine('{yellow}You have not learned Cure Poison yet.{/}');
    return;
  }
