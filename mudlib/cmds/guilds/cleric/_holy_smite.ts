/**
 * Holy Smite - Cleric combat skill.
 * Call down holy fire upon your enemies.
 */
...
export const name = ['holy_smite', 'holysmite', 'smite'];
export const description = 'Call down holy fire upon your enemies';
export const usage = 'holy_smite';

export async function execute(ctx: CommandContext): Promise<void> {
  const guildDaemon = getGuildDaemon();
  const player = ctx.player as GuildPlayer;
  const skillId = 'cleric:holy_smite';

  if (!guildDaemon.hasSkill(player, skillId)) {
    ctx.sendLine('{yellow}You have not learned Holy Smite yet.{/}');
    return;
  }
