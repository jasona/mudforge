/**
 * Group Heal - Cleric combat skill.
...
export const name = ['group_heal', 'groupheal', 'gh'];
export const description = 'Heal all allies in the room';
export const usage = 'group_heal';

export async function execute(ctx: CommandContext): Promise<void> {
  const guildDaemon = getGuildDaemon();
  const player = ctx.player as GuildPlayer;
  const skillId = 'cleric:group_heal';

  if (!guildDaemon.hasSkill(player, skillId)) {
    ctx.sendLine('{yellow}You have not learned Group Heal yet.{/}');
    return;
  }
