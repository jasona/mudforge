/**
 * Resurrection - Cleric utility skill.
...
export const name = ['resurrection', 'resurrect', 'rez'];
export const description = 'Return a dead player to life';
export const usage = 'resurrection <target>';

export async function execute(ctx: CommandContext): Promise<void> {
  const guildDaemon = getGuildDaemon();
  const player = ctx.player as GuildPlayer;
  const skillId = 'cleric:resurrection';

  if (!guildDaemon.hasSkill(player, skillId)) {
    ctx.sendLine('{yellow}You have not learned Resurrection yet.{/}');
    return;
  }

  const targetName = ctx.args.trim();
  if (!targetName) {
    ctx.sendLine('{yellow}Usage: resurrection <target>{/}');
    return;
  }
