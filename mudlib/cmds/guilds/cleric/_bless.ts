/**
 * Bless - Cleric buff skill.
 * Bless a target, increasing their accuracy.
 */
...
export const description = 'Bless a target, increasing their accuracy';
export const usage = 'bless [target]';

export async function execute(ctx: CommandContext): Promise<void> {
  const guildDaemon = getGuildDaemon();
  const player = ctx.player as GuildPlayer;
  const skillId = 'cleric:bless';

  if (!guildDaemon.hasSkill(player, skillId)) {
    ctx.sendLine('{yellow}You have not learned Bless yet.{/}');
    return;
  }
