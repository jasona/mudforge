/**
 * Bash - Fighter combat skill.
 * A powerful strike that deals extra damage and may stun the target.
 */
...
export const description = 'A powerful strike that deals extra damage';
export const usage = 'bash [target]';

export async function execute(ctx: CommandContext): Promise<void> {
  const guildDaemon = getGuildDaemon();
  const player = ctx.player as GuildPlayer;
  const skillId = 'fighter:bash';

  if (!guildDaemon.hasSkill(player, skillId)) {
    ctx.sendLine('{yellow}You have not learned Bash yet.{/}');
    return;
  }
