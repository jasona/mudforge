/**
 * Cleave - Fighter combat skill.
 * A sweeping attack that hits all enemies in the room.
 */
...
export const name = ['cleave'];
export const description = 'A sweeping attack that hits all enemies in the room';
export const usage = 'cleave';

export async function execute(ctx: CommandContext): Promise<void> {
  const guildDaemon = getGuildDaemon();
  const player = ctx.player as GuildPlayer;
  const skillId = 'fighter:cleave';

  if (!guildDaemon.hasSkill(player, skillId)) {
    ctx.sendLine('{yellow}You have not learned Cleave yet.{/}');
    return;
  }
