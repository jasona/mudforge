/**
 * Taunt - Fighter debuff skill.
 * Force an enemy to attack you for 6 seconds.
 */
...
export const name = ['taunt'];
export const description = 'Force an enemy to attack you';
export const usage = 'taunt [target]';

export async function execute(ctx: CommandContext): Promise<void> {
  const guildDaemon = getGuildDaemon();
  const player = ctx.player as GuildPlayer;
  const skillId = 'fighter:taunt';

  if (!guildDaemon.hasSkill(player, skillId)) {
    ctx.sendLine('{yellow}You have not learned Taunt yet.{/}');
    return;
  }
