/**
 * Assassinate - Thief combat skill.
 * A lethal strike that deals devastating damage to unsuspecting targets.
 */
...
export const name = ['assassinate'];
export const description = 'A lethal strike that deals devastating damage';
export const usage = 'assassinate [target]';

export async function execute(ctx: CommandContext): Promise<void> {
  const guildDaemon = getGuildDaemon();
  const player = ctx.player as GuildPlayer;
  const skillId = 'thief:assassinate';

  if (!guildDaemon.hasSkill(player, skillId)) {
    ctx.sendLine('{yellow}You have not learned Assassinate yet.{/}');
    return;
  }
