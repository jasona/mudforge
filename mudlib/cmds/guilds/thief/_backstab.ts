 ...
 * A devastating strike from behind that deals massive damage.
 */
...
export const name = ['backstab', 'bs'];
export const description = 'A devastating strike from behind';
export const usage = 'backstab [target]';

export async function execute(ctx: CommandContext): Promise<void> {
  const guildDaemon = getGuildDaemon();
  const player = ctx.player as GuildPlayer;
  const skillId = 'thief:backstab';

  if (!guildDaemon.hasSkill(player, skillId)) {
    ctx.sendLine('{yellow}You have not learned Backstab yet.{/}');
    return;
  }
