...
export const name = ['lockpick', 'pick'];
export const description = 'Attempt to pick a lock';
export const usage = 'lockpick <target>';

export async function execute(ctx: CommandContext): Promise<void> {
  const guildDaemon = getGuildDaemon();
  const player = ctx.player as GuildPlayer;
  const skillId = 'thief:lockpick';

  if (!guildDaemon.hasSkill(player, skillId)) {
    ctx.sendLine('{yellow}You have not learned Lockpick yet.{/}');
    return;
  }

  const targetName = ctx.args.trim();
  if (!targetName) {
    ctx.sendLine('{yellow}Usage: lockpick <target>{/}');
    return;
  }
